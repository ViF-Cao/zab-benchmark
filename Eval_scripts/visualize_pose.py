...
def plot_position(timestamps, x_vals, y_vals, z_vals):
...
def plot_3d_position(x_vals, y_vals, z_vals):
    fig = plt.figure(figsize=(10, 8))
    ax = fig.add_subplot(111, projection='3d')
    ax.plot(x_vals, y_vals, z_vals, label='position (x, y, z)', color='b')
...
def main():
    yaml_file = 'zabbox_groundtruth.yaml'
    data = read_yaml_file(yaml_file)
    timestamps, x_vals, y_vals, z_vals = extract_position_data(data)
    #plot_position(timestamps, x_vals, y_vals, z_vals)
    plot_3d_position(x_vals, y_vals, z_vals)
